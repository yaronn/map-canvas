...
Map.prototype.getMapData = function() {
   ...
   if (!this.options.excludeAntartica) {
      for (var i=0; i<antartica.shapes.length; i++) {
         json.shapes.push(antartica.shapes[i])
      }
   }
...
Map.prototype.drawLandMass = function() {   
   ...
   for (iShapeCounter = 0; iShapeCounter < landMass.shapes.length; iShapeCounter++) {
      ...
      // Fill the path green
      if (!this.options.disableFill) this.ctx.fill();
      this.ctx.stroke();
...
/*
Map.prototype.plotPosition = function(position) {
   this.ctx.beginPath();
...
Map.prototype.draw = function() {
   ...
   if (!this.options.disableMapBackground) this.drawMapBackground();

   // Draw the map background
   if (!this.options.disableGraticule) this.drawGraticule();
